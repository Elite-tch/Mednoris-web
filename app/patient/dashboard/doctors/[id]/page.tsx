...
import {
  ...
  Award, Building, Clock, Calendar,
  BadgeCheckIcon, Stethoscope, CalendarDays,
  MessageCircle
} from "lucide-react";
...
export default function PatientDoctorProfilePage() {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 px-4">
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 text-center max-w-md w-full">
          ...
          <button
            onClick={() => router.back()}
            className="w-full py-3 bg-brand-primary text-white font-bold rounded-xl hover:bg-brand-secondary transition-colors cursor-pointer"
          >
            Go Back
          </button>
        </div>
  ...
  const handleBookNow = () => {
    ...
    if (!user) {
      setToast({ message: "You must be logged in to book.", type: "error" });
      return;
    }
    router.push(`/patient/dashboard/doctors/${params.id}/book?slot=${encodeURIComponent(selectedSlot)}`);
  };
  ...
  return (
    <div className="pb-20 w-full">
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        <button
          onClick={() => router.push(`/patient/dashboard/chat?doctorId=${doctor.id}`)}
          className="flex items-center gap-2 px-4 py-2 bg-brand-secondary text-white hover:bg-brand-secondary hover:text-white font-bold rounded-xl transition-colors cursor-pointer shadow-sm"
        >
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-3xl p-6 sm:p-8 shadow-sm border border-gray-100">
            <div className="flex flex-col sm:flex-row gap-6 items-start">
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap items-center gap-3 text-sm text-gray-500 font-medium mb-4 mt-2">
              ...
              <div>
                ...
                <ul className="space-y-2 text-sm text-gray-600">
                  <li className="flex items-start gap-2 min-w-0">
                    <div className="w-1.5 h-1.5 rounded-full bg-brand-secondary shrink-0 mt-1.5"></div>
                    <span className="break-words min-w-0">
                      License No: <span className="font-bold text-gray-900 break-all">{doctor.medicalLicenseNumber}</span>
                    </span>
                  </li>
                </ul>
              </div>
          ...
          <div className="sticky top-6 space-y-6">
            ...
            >
                      ...
                      <div key={day} className="pb-3 border-b border-gray-50 last:border-0 last:pb-0">
                        ...
                        <div className="flex flex-col gap-2">
                          {dayData.slots.map((slot: any, i: number) => {
                            ...
                            // If slot has specific dates, render one radio per date
                            if (slotDates.length > 0) {
                              return slotDates.map((date) => {
                                ...
                                return (
                                  <label
                                    key={slotId}
                                    className={`flex items-center justify-between w-full px-3 py-2.5 rounded-xl cursor-pointer transition-all border ${
                                      isSelected
                                  ...
                                  >
                                    <div className="flex flex-col min-w-0">
                                      ...
                                      <span className={`text-sm font-bold ${isSelected ? "text-brand-primary" : "text-gray-700"}`}>
                                        {slot.start} – {slot.end}
                                      </span>
                                    </div>
                                    <input
                            ...
                            return (
                              ...
                              >
                                <span className={`font-bold text-sm ${isSelected ? "text-brand-primary" : "text-gray-700"}`}>
                                  {slot.start} – {slot.end}
                                </span>
                                <input
                                  type="radio"
              ...
              <button
                ...
                className={`w-full py-4 text-md text-white font-bold rounded-xl transition-all shadow-lg flex items-center justify-center gap-2 ${
                  getActiveDaysCount() === 0
                    ? "bg-gray-300 shadow-none cursor-not-allowed"
                    : "bg-brand-primary hover:bg-brand-secondary hover:shadow-brand-secondary/25 cursor-pointer"
                }`}
              >
                <Clock size={18} />
